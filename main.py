...
if __name__ == '__main__':
    ...
    for language in languages:
        ...
        all_expected_salaries_hh = list(filter(lambda x: x, map(predict_rub_salary_hh, language_vacancies_hh)))
        table_hh: dict = get_language_statistic(language_vacancies_hh, all_expected_salaries_hh, language)
        table_statistic_hh.append(table_hh)
        
        language_vacancies_sj = list(get_language_vacancies_sj(language, api_key))
        all_expected_salaries_sj = list(filter(lambda x: x, map(predict_rub_salary_sj, language_vacancies_sj)))
        table_sj: dict = get_language_statistic(language_vacancies_sj, all_expected_salaries_sj, language)
